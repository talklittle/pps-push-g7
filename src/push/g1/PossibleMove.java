...
public class PossibleMove extends Move {
	...
	public int getChangeInPoints(){
		...
		return board.getChangeInPoints(this);
	}
	...
	public boolean tooManyCoins() {
		int newcoins = 0;
		newcoins += board.getHexagon(this.getNewX(), this.getNewY())
				.getNumCoins();
	...
	public void setRelationshipRank(double d) {
		relationshipRank = d;
	}

	/**Boolean to let us know if we are really cooperating with who we are trying to*/
	public boolean getCooperation() {
		
		return (board.getHexAtPoint(this.getX(), this.getY()).getMultiplier() <= board
				.getHexAtPoint(this.getNewX(), this.getNewY()).getMultiplier());
	}
	
	/**Returns the hexagon representing the new space*/
	public Hexagon getNewHexagon(){
		return board.getHexagon(this.getNewX(), this.getNewY());
	}

	private PlayerBoard board;
	private Logger log;
	private final int cointhreshold = 10;
	private double relationshipRank;
